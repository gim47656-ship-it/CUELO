...
test("uses compositor-friendly session activity indicators", () => {
  assert.match(source, /className="session-running-spinner"/);
  assert.match(source, /className="session-unread-ring"/);
  assert.doesNotMatch(source, /<animate(?:Transform)?\b/);
});
...
test("refreshes the native priority mode every fifteen seconds", () => {
  assert.match(source, /window\.setInterval\(refreshPriority, PRIORITY_REFRESH_MS\)/);
  assert.match(source, /if \(navigatorMode !== "priority"\) return;/);
  assert.match(source, /loadSessions\(false, true\)/);
});

test("exposes labelled navigator controls and the current priority session", () => {
  assert.match(source, /role="toolbar"\s+aria-label=\{t\("navigator\.toolbar"\)\}/);
  assert.match(source, /aria-pressed=\{active\}/);
  assert.match(source, /data-navigator-mode=\{mode\}/);
