...
function UserMessageView({ message, cwd, onOpenFile, entryId, onFork, forking, onNavigate, prevAssistantEntryId, onEditContent }: {
  ...
  const rawContent =
    typeof message.content === "string"
      ...
      : message.content
          ...
          .map((b) => b.text)
          .join("\n");
  const documentPrompt = parseDocumentPrompt(rawContent);
  const content = documentPrompt?.message ?? rawContent;
  const attachedDocuments = documentPrompt?.documents ?? [];
  ...
  const copyContent = () => {
    copyText(getDocumentPromptDisplayText(rawContent)).then(() => {
      setCopied(true);
        ...
        >
              ...
              {imageBlocks.map((img, i) => {
                ...
                return (
                  ...
                  <img
                    ...
                    src={src}
                    alt=""
                    style={{ maxWidth: 240, maxHeight: 240, borderRadius: "var(--radius-control)", objectFit: "contain", display: "block", border: "1px solid var(--border)" }}
                  />
                );
              })}
            </div>
          ...
          {attachedDocuments.length > 0 && (
            <div
              ...
              style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: content ? 8 : 0 }}
            >
              {attachedDocuments.map((document, index) => (
                <div
                  ...
                  style={{
                    ...
                    border: "1px solid var(--accent-line)",
                    borderRadius: "var(--radius-control)",
                    background: "color-mix(in srgb, var(--user-bg) 70%, var(--bg-panel))",
                  }}
                >
                  {getFileIcon(document.name, 14)}
                  <span title={document.name} style={{ minWidth: 0, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 12 }}>
                    {document.name}
                  </span>
...
function AssistantMessageView({
  ...
  return (
    <div
      className={`message-assistant${isStreaming ? " is-streaming" : ""}`}
      style={{ marginBottom: 16 }}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      {/* 누가 이 답을 만들었는가: 계정(얼굴 + 별칭) 다음에 provider · 모델.
          계정을 특정하지 못하면 앞의 둘이 빠지고 provider · 모델만 남는다. 얼굴이 줄의
          중심이므로 별칭은 한 단계 크게, provider · 모델은 보조 크기로 남겨 위계를 만든다.
          줄 높이는 얼굴이 있을 때만 얼굴 크기로 잡는다 — 얼굴이 없는 기록까지 그 높이를
          비워 두면 옛 대화가 통째로 성기게 벌어진다. */}
      <div
        className="message-assistant-role"
        style={{
          fontSize: 11,
          color: "var(--text-dim)",
          marginBottom: 2,
      ...
      >
        {message.provider && (
          <>
            {accountFace && <AccountAvatar seed={accountFace.seed} size={40} provider={message.provider} />}
            {accountFace && (
        ...
        {/* Live token/throughput telemetry is not a per-message label; it
      ...
      <div style={{
        ...
        {message.usage && !isStreaming && (
          <button
            ...
            title={t("i18n.messageUsageTitle")}
            style={{
              padding: "3px 8px", height: 22,
              background: "none", border: "none", borderRadius: 5,
              color: usageOpen ? "var(--text-muted)" : "var(--text-dim)",
              cursor: "pointer",
              fontSize: 11, fontWeight: 400,
              whiteSpace: "nowrap",
              opacity: hovered || usageOpen ? 1 : 0,
              pointerEvents: hovered || usageOpen ? "auto" : "none",
              transition: "opacity 0.12s, color 0.12s",
            }}
          >
            {usageOpen ? formatUsage(message.usage) : t("i18n.messageUsage")}
          </button>
        )}
        {isStreaming && Math.round(estimatedTokens) > 0 && (
          <button
            type="button"
            onClick={() => setUsageOpen((open) => !open)}
            aria-expanded={usageOpen}
          ...
          >
            {usageOpen
              ? `~${Math.round(estimatedTokens)}${tps !== null ? ` · ${tps.toFixed(1)} t/s` : ""}`
              : t("i18n.messageUsage")}
          </button>
        )}
...
/**
 * An image the agent produced is part of the answer it wrote, so it renders in the transcript the
 * same way an attached image does - same `source` shapes, same element - only sized for the wider
 * answer column instead of a user bubble.
 */
...
function ToolCallBlock({ block, result, duration }: { block: ToolCallContent; result?: ToolResultMessage; duration?: number }) {
  ...
  const todoPhases = isTodoTool ? getTodoPhases(block, result) : null;
  ...
  return (
    <div
      className={`tool-call-card${isError ? " is-error" : " is-success"}`}
      style={{
        borderRadius: 7,
        overflow: "hidden",
        fontSize: 12,
        border: isError ? "1px solid var(--danger-line)" : "1px solid var(--success-line)",
    ...
    >
      ...
      >
        ...
        <span className="tool-call-name" style={{ color: isError ? "var(--danger)" : "var(--success)", fontFamily: "var(--font-mono)", fontWeight: 600, fontSize: 11, flexShrink: 0 }}>
          {block.toolName}
        </span>
      ...
      {resultDiff && (
        <div style={{ maxHeight: expanded ? 560 : 260, overflowY: "auto", overflowX: "hidden", borderTop: "1px solid var(--success-line)" }}>
      ...
      {expanded && !hasStructuredPreview && (
        <pre
          ...
          style={{
            ...
            borderTop: isError ? "1px solid var(--danger-line)" : "1px solid var(--success-line)",
            whiteSpace: "pre-wrap",
            wordBreak: "break-all",
          }}
        >
...
function TodoChecklistPreview({ phases }: { phases: TodoPhase[] }) {
...
function getStructuredToolPreview(block: ToolCallContent, phases: TodoPhase[] | null): string {
  if (phases) {
...
function PairedDiffResult({ diff }: {
  ...
  return (
    <div
      style={{
        borderTop: "1px solid var(--success-line)",
        background: "var(--bg)",
      }}
    >
    ...
    <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, lineHeight: 1.55, minWidth: 0 }}>
      {lines.map((line, i) => {
        const kind =
          ...
          "context";
        const bg =
          kind === "added" ? "var(--success-soft)" :
          kind === "removed" ? "var(--danger-soft)" :
...
function PairedResult({ text, isEmpty, isError }: {
  ...
  return (
    <div
      style={{
        borderTop: `1px solid ${isError ? "var(--danger-line)" : "var(--success-line)"}`,
        background: isError ? "var(--danger-soft)" : "var(--bg-subtle)",
      }}
    >
      <pre
        style={{
...
function getToolPreview(block: ToolCallContent): string {
  ...
  // 앞의 특수 경로는 모두 문자열 필드를 노린다. 거기 걸리지 않은 도구는 값이 객체일 수 있고
  // (`yield` 의 입력이 `{ data: {...}, type }` 이다) 그대로 문자열로 만들면 `[object Object]` 가
  // 그려진다. 그래서 원시값을 가진 첫 키를 먼저 찾고, 없을 때만 구조를 요약한다.
  const primitive = keys
    .map((key) => input[key])
    .find((value) => typeof value === "string" || typeof value === "number" || typeof value === "boolean");
  if (primitive !== undefined) return String(primitive).slice(0, 120);
  try {
    return JSON.stringify(input).slice(0, 120);
  } catch {
    return keys.join(", ").slice(0, 120);
  }
}
