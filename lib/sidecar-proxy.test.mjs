...
test("a second CUELO on other ports pairs the proxy with its own loopback sidecars", async () => {
  const names = ["PORT", "OMP_USAGE_PORT", "OMP_BTW_PORT", "OMP_SUBAGENT_PORT"];
  const saved = Object.fromEntries(names.map((name) => [name, process.env[name]]));
  Object.assign(process.env, { PORT: "31141", OMP_USAGE_PORT: "31142", OMP_BTW_PORT: "http://attacker.example", OMP_SUBAGENT_PORT: "31144" });
  try {
    // Origins are read at module load, so this needs its own module instance.
    const other = await import("./sidecar-proxy.ts?second-port-set");
    assert.equal(other.resolveSidecarTarget("GET", "resource", ["usage"])?.url, "http://127.0.0.1:31142/usage");
    assert.equal(other.resolveSidecarTarget("POST", "sidechat", ["ask"])?.url, "http://127.0.0.1:30143/ask");
    assert.equal(other.resolveSidecarTarget("GET", "subagent", ["archive"])?.url, "http://127.0.0.1:31144/archive");
    assert.equal(other.resolveSidecarTarget("POST", "resource", ["credential", "27", "reset"])?.origin, "http://127.0.0.1:31141");
    assert.equal(other.resolveSidecarTarget("GET", "subagent", ["unknown"]), null);
  } finally {
