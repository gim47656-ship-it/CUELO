...
const { formatLiveTranscriptContent, LIVE_TRANSCRIPT_MESSAGE_TYPE } = await jiti.import("./live-types.ts");
const {
  listAllSessions,
  mergeSessionLists,
  buildSessionContext,
...
const { GPT6_REPLY_CUSTOM_TYPE, GPT6_REPLY_SOURCE } = await jiti.import("./gpt6-bridge.ts");
...
// 6PRO 답변은 사용자가 읽으러 온 답이다. custom으로 남으면 user/assistant가 아닌
// 메시지를 작업 로그로 보내는 규칙에 걸려 「작업 과정」 탭으로 들어간다.
test("keeps published 6PRO replies in the conversation, not the work log", () => {
  const entries = [
    userEntry("u1", null, "티보알어?"),
    {
      type: "custom_message",
      id: "08276e04",
      parentId: "u1",
      timestamp: "2026-01-01T00:00:02.000Z",
      customType: GPT6_REPLY_CUSTOM_TYPE,
      content: "원인은 투영 손실입니다.",
      display: true,
      details: { source: GPT6_REPLY_SOURCE, requestId: "q-17" },
    },
  ];
  ...
  assert.deepEqual(context.messages[1], {
    role: "assistant",
    content: [{ type: "text", text: "원인은 투영 손실입니다." }],
  ...
  const { main, process } = partitionTranscriptPlan(context.messages, buildTranscriptRenderPlan(context.messages));
  const answers = main
    .filter((item) => item.kind === "answer")
    .flatMap((item) => item.blocks.map((block) => block.text));
  assert.deepEqual(answers, ["원인은 투영 손실입니다."]);
  assert.deepEqual(process, []);
});

test("projects persisted live voice turns into attributed conversation messages", () => {
  const entries = [
    {
      type: "custom_message",
      id: "voice-user",
      ...
      content: formatLiveTranscriptContent("user", "회의 일정 확인해 줘"),
      display: true,
      details: { role: "user" },
      attribution: "user",
    },
    {
      ...
      customType: LIVE_TRANSCRIPT_MESSAGE_TYPE,
      content: formatLiveTranscriptContent("assistant", "내일 오전 일정부터 볼게요."),
      display: true,
  ...
  assert.deepEqual(context.entryIds, ["voice-user", "voice-assistant"]);
  ...
  assert.deepEqual(context.messages[1], {
    role: "assistant",
    content: [{ type: "text", text: "내일 오전 일정부터 볼게요." }],
    model: "Codex Live",
...
test("reads only a bounded session header, including headers larger than 4 KiB", () => {
  const dir = mkdtempSync(join(tmpdir(), "cuelo-header-"));
...
test("returns null for malformed or unbounded session headers", () => {
  const dir = mkdtempSync(join(tmpdir(), "cuelo-header-invalid-"));
...
test("keeps forward and reverse session path caches in sync", async () => {
  ...
  const filePath = join(tmpdir(), "cuelo-cache-test", "..", "cache-test", "session.jsonl");
...
test("drops cached paths for transient sessions that were never persisted", async (t) => {
  ...
  const dir = mkdtempSync(join(tmpdir(), "cuelo-transient-session-"));
  const filePath = join(dir, "session.jsonl");
...
test("hides extracted document text from persisted session listings", async (t) => {
  const originalListAll = SessionManager.listAll;
  const firstMessage = composeDocumentPrompt("Review the attachment", [{
    name: "manual.pdf",
  ...
  SessionManager.listAll = async () => [{
    path: "V:/sessions/document.jsonl",
    id: "document-session",
    cwd: "",
    title: undefined,
  ...
  assert.equal(sessions[0].firstMessage, "Review the attachment");
});
