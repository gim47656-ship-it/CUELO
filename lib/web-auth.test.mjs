...
async function withStore(run) {
  ...
  try {
    return await run({ file: join(dir, "cuelo-auth.json"), env: {}, params: PARAMS });
  } finally {
...
test("authorizes against CUELO_PASSWORD ahead of the stored password", async () => {
  const { authorizeWebRequest } = await loadSubject();
  await withStore(async (options) => {
    store.setWebPassword(STORED_PASSWORD, options);
    const withEnv = { ...options, env: { CUELO_PASSWORD: "environment-secret" } };
