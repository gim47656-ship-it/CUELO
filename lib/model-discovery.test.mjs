...
before(async () => {
  ...
  previousRuntime = globalThis.__ompRuntimePromise;
  globalThis.__ompRuntimePromise = Promise.resolve({ authStorage });
});
after(() => {
...
test("resolves environment-backed headers without an API key", async () => {
  process.env.CUELO_DISCOVERY_TEST_TOKEN = "resolved-token";
  try {
    const auth = await resolveModelDiscoveryAuth("cuelo-header-only-test", {
      baseUrl: "https://example.invalid/v1",
      api: "openai-completions",
      headers: { "X-Discovery-Token": "CUELO_DISCOVERY_TEST_TOKEN" },
    });
    assert.equal(auth.apiKey, undefined);
    assert.deepEqual(auth.headers, { "X-Discovery-Token": "resolved-token" });
  } finally {
    delete process.env.CUELO_DISCOVERY_TEST_TOKEN;
  }
});

test("discovers with an existing provider key without requiring an inline model key", async () => {
  authStorage.keys.setRuntime("discovery-stored-key", "fixture-provider-key");
  const auth = await resolveModelDiscoveryAuth("discovery-stored-key", {
    baseUrl: "https://example.invalid/v1",
    api: "openai-completions",
    auth: "apiKey",
    discovery: { type: "openai-models-list" },
  });
  assert.equal(auth.apiKey, "fixture-provider-key");
