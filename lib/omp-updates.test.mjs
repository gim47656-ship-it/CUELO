...
test("announces a newer CUELO release with its changelog", async () => {
  const status = await getOmpWebUpdateStatus({ currentAppVersion: "0.1.7", fetcher: fetcher(release) });
...
test("stays quiet when the latest release is not newer", async () => {
  const status = await getOmpWebUpdateStatus({ currentAppVersion: "0.1.8", fetcher: fetcher(release) });
...
test("treats a repository without releases as up to date", async () => {
  const status = await getOmpWebUpdateStatus({ currentAppVersion: "0.1.7", fetcher: fetcher(null, 404) });
...
test("reports an unavailable update source as an error", async () => {
  await assert.rejects(
    getOmpWebUpdateStatus({ currentAppVersion: "0.1.7", fetcher: fetcher(null, 503) }),
    /HTTP 503/,
  );
});
