...
test("resolves only the most recent assistant identity and never invents a provider", () => {
  assert.deepEqual(latestSideChatAssistantIdentity({
    context: {
      messages: [
        { role: "assistant", provider: "openai-codex", credentialId: 4 },
        { role: "user", content: "later user entry" },
        { role: "assistant", provider: "anthropic", credentialId: 9 },
      ],
    },
  }), { provider: "anthropic", credentialId: 9 });
  assert.equal(latestSideChatAssistantIdentity({
    context: { messages: [{ role: "assistant", provider: "openai-codex" }, { role: "assistant" }] },
  }), null);
  assert.equal(latestSideChatAssistantIdentity({ context: { messages: [{ role: "user" }] } }), null);
  assert.equal(latestSideChatAssistantIdentity(null), null);
});

test("assembles a clearly separated main-session message with optional instruction", () => {
  assert.equal(
    buildSideChatForwardMessage("질문", "답변", "계속 조사해 줘"),
    "사이드채팅 문답을 메인 대화에서 이어서 처리해 주세요.\n\n[사이드채팅 질문]\n질문\n\n[사이드채팅 답변]\n답변\n\n[추가 지시]\n계속 조사해 줘",
  );
  assert.equal(
    buildSideChatForwardMessage("질문", "답변", " \n "),
    "사이드채팅 문답을 메인 대화에서 이어서 처리해 주세요.\n\n[사이드채팅 질문]\n질문\n\n[사이드채팅 답변]\n답변",
  );
});

test("selects steer only while the main session is running", () => {
  assert.deepEqual(sideChatForwardCommand(true, "message"), { type: "steer", message: "message" });
  assert.deepEqual(sideChatForwardCommand(false, "message"), { type: "prompt", message: "message" });
});

test("claims each answer once and permits retry only after a failed send", () => {
  const registry = createSideChatForwardRegistry();
  assert.equal(registry.claim("session:turn"), true);
  assert.equal(registry.claim("session:turn"), false);
  registry.release("session:turn");
  assert.equal(registry.claim("session:turn"), true);
  registry.markSent("session:turn");
  assert.equal(registry.claim("session:turn"), false);
});
test("uses get_state activity, not the alive wrapper flag, to choose steer or prompt", () => {
  const idleAlive = {
    running: true,
    state: {
      isStreaming: false,
  ...
  assert.equal(isMainSessionBusy(idleAlive), false);
  assert.equal(isMainSessionBusy({ running: false }), false);
  assert.equal(isMainSessionBusy({
    ...idleAlive,
    state: { ...idleAlive.state, isStreaming: true },
  }), true);
  // 끝난 자식이 남아 있어도 메인 턴이 없으면 steer가 아니라 prompt로 보내야 한다.
  assert.equal(isMainSessionBusy({
    ...idleAlive,
    state: { ...idleAlive.state, subagents: [{ status: "completed" }] },
  }), false);
  assert.equal(isMainSessionBusy({ running: true, state: {} }), null);
});
