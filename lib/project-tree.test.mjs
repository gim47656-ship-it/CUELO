...
const { composeDocumentPrompt } = await import("./document-attachments.ts");
...
test("hides extracted document context from branch previews", () => {
  ...
  const arm = node(msg("u-doc", "user", prompt), [node(info("leaf-doc"))]);
  const sibling = node(msg("u-other", "user", "Other branch"), [node(info("leaf-other"))]);
  const projected = projectTreeForResponse([node(info("root"), [arm, sibling])]);

  assert.deepEqual(findProjectedNode(projected, "leaf-doc").branchPreview, {
    role: "user",
    text: "Review the attachment",
  });
  assert.equal(JSON.stringify(projected).includes("private extracted body"), false);
});
