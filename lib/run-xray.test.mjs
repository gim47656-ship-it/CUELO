...
// 임시 HOME에 세션 파일과 stats.db를 만드는 테스트들이다. 평소 CI에서 0.3~0.5초지만,
// 러너가 느린 순간(run 36275079772·36276227870)에는 7~10초가 걸려 bun 기본 5초를 넘겼다.
// 다시 돌리면 코드 변경 없이 통과했으므로 제한만 넉넉히 둔다.
const FIXTURE_TEST = { timeout: 30_000 };

test("요청 timestamp를 생성 시작으로 해석해 겹침과 창 경계를 올바르게 센다", FIXTURE_TEST, () => {
...
test("자식 transcript를 역할로 잡고 창 시작 시점 effort를 이어받는다", FIXTURE_TEST, () => {
...
test("이름이 겹쳐 -N이 붙은 child도 같은 task의 역할로 귀속한다", FIXTURE_TEST, () => {
...
test("경계 시각의 effort 변경은 다음 run 것이고 마지막 도구 호출까지 경과에 든다", FIXTURE_TEST, () => {
  const fixture = makeFixture();
  try {
    const runs = listRuns(SESSION_ID).runs;
...
test("창이 닫힌 뒤의 effort 변경은 마지막 run에 들어가지 않는다", FIXTURE_TEST, () => {
  const fixture = makeFixture();
  try {
    const runs = listRuns(SESSION_ID).runs;
...
test("advisor는 파일명 규약으로만 잡고 소요시간 미기록을 하한으로 드러낸다", FIXTURE_TEST, () => {
  const fixture = makeFixture();
  try {
    const runs = listRuns(SESSION_ID).runs;
...
test("다음 요청이 창을 닫으면 running이 아니라 interrupted다", FIXTURE_TEST, () => {
  const fixture = makeFixture();
  try {
    const runs = listRuns(SESSION_ID).runs;
...
test("구성 비교는 자식 transcript를 독립 세션으로 세지 않는다", FIXTURE_TEST, () => {
  const fixture = makeFixture();
  try {
...
test("attributed 범위는 역할을 확정하지 못한 run을 그룹화 전에 뺀다", FIXTURE_TEST, () => {
...
test("attributed 범위는 역할이 하나도 없는 run도 제외한다", FIXTURE_TEST, () => {
...
test("xai-oauth 구독 사용량은 0원이 아니라 미가격으로 계수한다", FIXTURE_TEST, () => {
...
test("카탈로그에 없는 provider의 0원은 미가격으로 오분류하지 않는다", FIXTURE_TEST, () => {
