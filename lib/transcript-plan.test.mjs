...
function commandOutput(content, timestamp) {
  return { role: "custom", customType: "command", content, display: true, timestamp };
}

test("keeps a slash command result in the conversation of its turn", async () => {
  const { buildTranscriptRenderPlan, partitionTranscriptPlan } = await loadSubject();
  const messages = [
    user("go"),
    assistant([toolCall("a")]),
    { role: "toolResult", toolCallId: "a", content: [text("ok")] },
    commandOutput("Fast mode is on."),
    assistant([text("Done.")]),
  ];
  for (const options of [{}, { sessionBusy: true }]) {
    const { main, process } = partitionTranscriptPlan(messages, buildTranscriptRenderPlan(messages, options));
    const command = main.find((item) => item.kind === "message" && item.idx === 3);
    assert.equal(command?.anchorIdx, 0, "the result belongs to the running turn, not a turn of its own");
    assert.equal(process.some((group) => group.entries.some((entry) => entry.idx === 3)), false);
  }
});

test("re-applies local command results after a transcript reload without duplicating them", async () => {
  const { withLocalCommandOutputs } = await loadSubject();
  const first = commandOutput("Fast mode is on.", 150);
  const second = commandOutput("Fast mode is on.", 150);
  const outputs = [
    { entryId: "local-command:1", baseEntryId: "u1", message: first },
    { entryId: "local-command:2", baseEntryId: "u1", message: second },
  ];
  const server = [
    { ...user("go"), timestamp: 100 },
    assistant([toolCall("a")], { timestamp: 150 }),
    assistant([text("Done.")], { timestamp: 200 }),
  ];
  const once = withLocalCommandOutputs(server, ["u1", "a1", "a2"], outputs);
  assert.deepEqual(once.messages, [server[0], server[1], first, second, server[2]], "ties stay after the server message, in arrival order");
  assert.deepEqual(once.entryIds, ["u1", "a1", "local-command:1", "local-command:2", "a2"]);

  const again = withLocalCommandOutputs(once.messages, once.entryIds, outputs);
  assert.deepEqual(again, once);

  const withTail = withLocalCommandOutputs([...server, first], ["u1", "a1", "a2"], outputs);
  assert.deepEqual(withTail, once, "a result still sitting in the live tail is moved, not copied");
});

test("does not carry a local command result into a branch without its base entry", async () => {
  const { withLocalCommandOutputs } = await loadSubject();
  const output = commandOutput("Fast mode is on.", 150);
  const branch = withLocalCommandOutputs(
    [{ ...user("other"), timestamp: 300 }],
    ["u2"],
    [{ entryId: "local-command:1", baseEntryId: "a1", message: output }],
  );
  assert.deepEqual(branch.entryIds, ["u2"]);
  assert.equal(branch.messages.includes(output), false);
});
