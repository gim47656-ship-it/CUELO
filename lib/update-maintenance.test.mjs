...
async function withExternalRoot(run) {
  ...
  const previous = process.env.OMPWEB_EXTERNAL_UPDATE_ROOT;
  process.env.OMPWEB_EXTERNAL_UPDATE_ROOT = root;
  try {
    await run(root);
  } finally {
    if (previous === undefined) delete process.env.OMPWEB_EXTERNAL_UPDATE_ROOT;
    else process.env.OMPWEB_EXTERNAL_UPDATE_ROOT = previous;
    delete process.env.OMPWEB_DEPLOY_REQUEST_ID;
    delete process.env.OMPWEB_DEPLOY_STAGE_HASH;
    delete process.env.OMPWEB_DEPLOY_MARKER_PATH;
...
test("공유 request/command receipt로 drain, parked, exact resume identity를 잇는다", async () => {
  await withExternalRoot(async (root) => {
    ...
    process.env.OMPWEB_DEPLOY_REQUEST_ID = requestId;
    process.env.OMPWEB_DEPLOY_STAGE_HASH = stageHash;
    process.env.OMPWEB_DEPLOY_MARKER_PATH = markerPath;
    ...
    const rollbackDirectory = join(root, "npm-prefix", ".omp-web-rollback-test");
    const rollbackPackage = join(rollbackDirectory, "omp-web");
    ...
    const cleanup = {
      ...
      updatedAtUtc: new Date().toISOString(),
      finishedAtUtc: null,
      currentTarget: ".omp-web-stage-old",
      completedCount: 0,
      totalCount: 2,
      removedCount: 0,
      keptCount: 0,
    ...
    assert.equal(released.cleanup.currentTarget, ".omp-web-stage-old");
    assert.equal(maintenance.registerUpdateClient({
      clientId,
      sessionId: "session-a",
      resumeUrl: "/?session=session-a",
    ...
    const cleanupFailed = {
      ...
      failures: [{ target: ".omp-web-rollback-old", reason: "사용 중" }],
    };
...
test("marker나 session identity가 다르면 resume receipt를 만들지 않는다", async () => {
  await withExternalRoot(async (root) => {
    ...
    process.env.OMPWEB_DEPLOY_REQUEST_ID = requestId;
    process.env.OMPWEB_DEPLOY_STAGE_HASH = stageHash;
    process.env.OMPWEB_DEPLOY_MARKER_PATH = join(root, "missing-marker.json");
...
async function setupCompletedDeployment(root, { requestId, stageHash, initiatorSessionId }) {
  ...
  const rollbackDirectory = join(root, "npm-prefix", `.omp-web-rollback-${requestId.slice(0, 8)}`);
  const rollbackPackage = join(rollbackDirectory, "omp-web");
  const rollbackShims = join(rollbackDirectory, "shims");
  const rollbackTransaction = join(rollbackDirectory, "transaction.json");
  await mkdir(rollbackPackage, { recursive: true });
...
test("실패 통지는 서버가 result로 실패를 재판정해 request당 한 번만 통과한다", async () => {
  await withExternalRoot(async (root) => {
    ...
    const terminalError = "OMPWEB drain 시간 초과 (120s): runningSessions=01a0c991-9053-75e5-933d-d77cff2d7e40 parked=0/0. live runtime은 변경하지 않았다.";
...
test("실패 통지는 살아 있는 idle 세션에 재기동 없이 바로 전달된다", { skip: WAKE_MODULE_SKIP }, async () => {
  await withExternalRoot(async (root) => {
    ...
    const terminalError = "OMPWEB drain 시간 초과 (120s): parked=0/0. live runtime은 변경하지 않았다.";
