...
test("lists directories and directory symlinks without returning files", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "cuelo-browse-"));
  try {
...
test("expands home-relative paths and rejects missing directories", async () => {
  ...
  await assert.rejects(resolveDirectory(path.join(tmpdir(), `cuelo-missing-${Date.now()}`)));
});
...
test("keeps a Windows drive root absolute after realpath resolution", async () => {
  const { normalizeResolvedDirectory } = await loadSubject();

  assert.equal(normalizeResolvedDirectory("V:"), "V:\\");
  assert.equal(normalizeResolvedDirectory("V:\\"), "V:\\");
  assert.equal(normalizeResolvedDirectory("V:\\Projects"), "V:\\Projects");
});
