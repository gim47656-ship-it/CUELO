...
const UNPDF_REQUEST = /^unpdf(?:\/.*)?$/;
/**
 * `bun:sqlite` is a Bun builtin with no npm package behind it. The server runs
 * on Bun, so the import resolves at runtime, but webpack must be told to leave
 * it alone or the server build fails to resolve the module.
 */
  ...
  webpack: (config, { isServer, nextRuntime }) => {
    ...
    config.externals = [
      ({ request }: { request?: string }, callback: (error?: unknown, result?: string) => void) => {
        ...
        if (request && UNPDF_REQUEST.test(request)) return callback(undefined, `import ${request}`);
        if (request && BUN_BUILTIN_REQUEST.test(request)) return callback(undefined, `import ${request}`);
        return callback();
      },
      ...externals,
    ];
