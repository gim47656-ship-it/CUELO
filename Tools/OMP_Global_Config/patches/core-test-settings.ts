...
// configured values through rawValue. The real 18.3.1 Settings has no get/getGroup, so this
// surface must not expose them either: 2026-09-25 patched core still called
// `settings.get("mnemopi.autoRetain")`, the harness mock answered it, and every live session
// start failed with HTTP 500.
 ...
 * 흉내 객체의 get/getGroup은 값 공급원으로만 쓰고 결과에서는 뺀다(실제 18.3.1 Settings에 없다).
 */
export function settingsLike<T extends { get: (key: string) => unknown; getGroup?: (group: string) => unknown }>(mock: T): T {
	...
	const surface = Object.assign(Object.create(Object.getPrototypeOf(mock)), mock, scope);
	delete surface.get;
	delete surface.getGroup;
	return surface as T;
}
