...
- 새 검증 서버가 실제로 필요하고 dev 파이프라인으로 그 트리를 띄울 수 없을 때만 omp-web의 격리 검증 서버를 **production build + start**로 띄운다. 적합한 동일 revision 빌드·서버·결과가 있으면 먼저 재사용하므로 이 절은 매 작업마다 production build를 요구하지 않는다(`implementationOwnership.writerValidation.surfaceSetup`·`reuseScope`). 이 트리에서 `next dev`는 그냥 못 쓴다 — `app/globals.css:13`의 `@import "@seed-design/css/base.css"`부터 JS로 파싱돼 `Module parse failed: Unexpected character '@'`가 나고 이어서 `node_modules/@seed-design/css/recipes/*.css`가 전부 깨진다. 2026-09-19 실측으로 두 조건을 갈랐다: `OMP_WEB_DIST_DIR`을 준 30152와 **아무 환경변수 없이 띄운 30153이 동일하게** `styleSheets 1 / --seed-color-bg-layer-default null / .chat-content-layout 없음 / textLen 0`이었다(정상은 `4 / #fff / 존재 / 1466`). **원인은 distDir이 아니라 dev 파이프라인 자체다.** 이 사실은 `CUELO_Setup/HANDOFF.md`의 「남은 일 24」에 이미 기록돼 있었다 — 검증 서버를 띄우기 전에 그 프로젝트의 HANDOFF를 먼저 읽으면 재조사가 필요 없다. 정상 절차는 `OMP_WEB_DIST_DIR=<dir> bun --bun next build --webpack` 뒤 `next start -p <port>`이고, 빌드는 `HOME`·`USERPROFILE`을 빈 임시 디렉터리로 주지 않으면 Next output tracer가 Windows 보호 junction을 따라가 `EPERM: scandir 'C:/Users/<user>/My Documents'`로 죽는다(이것도 「남은 일 24」의 ②). 이번 확인을 위해 새로 만든 일회성 검증 부산물은 필요한 확인이 끝난 뒤 정리하되, 기존 공유·재사용 가능한 유효 산출물은 소유권·수명 정책에 따라 보존한다. distDir을 바꾸면 Next가 자동 재작성하는 `tsconfig.json`의 `include`와 `next-env.d.ts`는 복원한다(커밋 대상이 아니다).
...
- CUELO 재시작은 작업을 보존한 뒤 `omp-web` 프로세스 계보 밖에서 수행한다. Git Bash·OMP `bash` tool 같은 비대화형 자동화는 `powershell.exe ... -File C:/Users/<user>/.omp/restart-ompweb.ps1`처럼 배치된 PowerShell 정본을 `/` 경로로 직접 실행한다. `restart-ompweb.cmd`는 ASCII·CRLF wrapper로 유지하고 Win+R 또는 `OMPWEB-Restart` 작업 스케줄러에서만 사용한다. 스케줄러의 접수 성공만으로 재시작을 판정하지 말고 재시작 로그와 실제 HTTP·브라우저 응답을 확인한다. PC 재부팅을 기본 해결책으로 요구하지 않는다.
